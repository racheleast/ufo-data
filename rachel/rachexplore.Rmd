...
NuclearData4 <- data.frame(NuclearData3$NuclearData..Reactor.Name., NuclearData3$NuclearData.Longitude - .5, NuclearData3$NuclearData.Longitude + .5, NuclearData3$NuclearData.Latitude - .5, NuclearData3$NuclearData.Latitude + .5)
...
colnames(newtable) = c("Reactor Name", "Count")
...
Notes: Something seems to have gone wrong. Our counts in the table seems very low and the total counts are only making up about 22% of our total sightings before 2005. 

Also, it seems to be harder to convert longitude/latitude to miles than we thought. Some sites are saying that one degree is about 60 miles which is why I did a plus/minus of .5 , but other sites say it depends on how close you are to the equator. I will definitely need to do some more reaserch on this. 

I have decided to zoom in on the map and check our count for one of the power plants. I am using the power plant "Diablo Canyon 1" which on the table has 8 counts, but as you will see on the map there are only 6 sightings in our range. 
...
(mapufo <- ggplot() +
    ...
    theme_classic() + # Remove Grey Background
    ylim(34.71157, 35.71157) +
    xlim(-121.35554, -120.35554) +
    xlab("Longitude") +
    ylab("Latitude") +
