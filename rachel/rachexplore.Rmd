...
mapufo <- ggplot() +
    ...
    geom_point(data = earlydata, aes(x = earlydata$data.city_longitude , y = earlydata$data.city_latitude, text = paste0(earlydata$data.sighting_year)) , color = "red", cex = .3 ) + # Plot Data
    geom_point(data = NuclearData3, aes(x = NuclearData3$NuclearData.Longitude , y = NuclearData3$NuclearData.Latitude, text = paste0(NuclearData3$NuclearData..Reactor.Name.)) , color = "blue", cex = 1) +
    theme_classic() + # Remove Grey Background
    ylim(25, 50) +
    xlim(-125,-65) +
    xlab("Longitude") +
...
##### Based on our visuals it seems like we have a positive correlation between the number of power plants and the number of sightings. California and South Carolina are the 2 states that have 7 power plants, but California has 650 sightings where as South Carolina has only 90. California is definitely the outlier of this data as the next highest number of sightings is 301 (Texas). Based on our "README" I think California has some other factors (San Andreas Fault, Los Angeles, Earthquakes) that contribute to more sightings. We will have to consider this as we continue our analysis. 
...
fig <- (mapufo <- ggplot() +
    geom_point(data = earlydata, aes(x = earlydata$data.city_longitude , y = earlydata$data.city_latitude, text = paste0(earlydata$data.sighting_year)) , color = "red", cex = 1 ) + # Plot Data
    geom_point(data = NuclearData3, aes(x = NuclearData3$NuclearData.Longitude , y = NuclearData3$NuclearData.Latitude, text = paste0(NuclearData3$NuclearData..Reactor.Name.)) , color = "blue", cex = 2 ) +
